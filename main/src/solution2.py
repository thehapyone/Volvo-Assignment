...
headers_service_info = ['id', 'time', 'version', 'product', 'backendRegion', 'xRequestId', 'privacyClass', 'flowId', 'contentCategory', 'requestId']
# Field Names for the Service_Dev table
headers_service_dev = ['id', 'application', 'applicationVersion', 'buildVersion', 'environment', 'origin', 'channel', 'path', 'method']
# Field Names for the Service_Content table
headers_service_content = ['requestId', 'serviceId', 'subId', 'vin', 'serviceProviderName', 'serviceMainType', 'serviceStatus',
...
def parse_content_table(data):
    ...
    result = [data[col] if col not in content_location_info else parse_location(data, col) for col in headers_service_content if
              col in data_keys]
